...
namespace MUSystem.Core
{
    public partial class ServiceBase<T> where T : ModelBase, new()
    {
        ...
        /// 系统级的只更新form数据的更新
        /// </summary>
        /// <param name="formWrapper"></param>
        /// <param name="data"></param>
        /// <returns></returns>
        public int EditForm(RequestWrapper formWrapper, JObject data)
        {
            #region 定义变量
        ...
        /// <param name="data"></param>
        /// <param name="isNotSaveEmpty">空数据不需要保存</param>
        /// <returns></returns>
        public int Edit(RequestWrapper formWrapper, RequestWrapper listWrapper, JObject data,int isNotSaveEmpty = 0)
        {
            #region 定义变量
            var types = new Dictionary<string,OptType>{
                {"deleted"  ,OptType.Del},
            ...
            Logger("编辑记录", () =>
            {
                ...
                {
                    editArgs.wrapper = formWrapper;
                    editArgs.formOld = this.GetDynamic(formWrapper.SetRequestData(editArgs.form, isNotSaveEmpty).ToParamQuery())??new JObject();
                    var rtnBefore = this.OnBeforEdit(editArgs);
                    if (!rtnBefore) return;
      
                    var pUpdate = formWrapper.SetRequestData(editArgs.form, isNotSaveEmpty).ToParamUpdate();
                    if (pUpdate.GetData().Columns.Count > 0)//更新主表
                    {
                        rtnBefore = this.OnBeforEditMaster(editArgs);
                        if (rtnBefore)
                        {
                            pUpdate = formWrapper.SetRequestData(editArgs.form, isNotSaveEmpty).ToParamUpdate(); //在before事件中更改了form中的值，刷新更新时的值
                            rowsAffected = BuilderParse(pUpdate).Execute();
                            if (rowsAffected == 0)
                                rowsAffected = BuilderParse(formWrapper.ToParamInsert()).Execute();
        ...
        public int EditList(RequestWrapper listWrapper, JToken data)
        {
            #region 定义变量
            var types = new Dictionary<string, OptType>{
                {"deleted"  ,OptType.Del},
            ...
            {
                db.UseTransaction(true); //开启事务
                var editArgs = new EditEventArgs() { db = db, list = data };


                if (data != null && listWrapper != null)
                {
                    editArgs.wrapper = listWrapper;

                    foreach (JProperty item in data.Children())
                    {
                        if (!handles.ContainsKey(item.Name))
                            continue;
