...
namespace MUSystem.Core
{
    public static class FormsAuth
    {
        ...
        public static void SignInSecond(string loginName, object userData, int expireSeconds)
        {
            var data = JsonConvert.SerializeObject(userData);
            ...
            if (expireSeconds > 0) { cookie.Expires = DateTime.Now.AddSeconds(expireSeconds); } //增加多少秒
