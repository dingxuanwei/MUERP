...
namespace MUSystem.Core
{
    public partial class RequestWrapper
    {
        ...
        public RequestWrapper SetRequestData(JToken values,int isNotSaveEmpty = 0)
        {
            if (values != null)
            {
                foreach (JProperty item in values.Children())
                {
                    if (isNotSaveEmpty == 1)//空数据不要保存
                    {
                        if (item != null && item.Value.ToString() != "") this[item.Name] = item.Value.ToString();
                    }
                    else
                         if (item != null ) this[item.Name] = item.Value.ToString();
                }
